...
def send_to_progress_report(english, finnish):
	...
	try:
		...
		while True:
			...
			data_to_send["realtime"] = [ server_name + ": " + realtime.replace('_', ' ')  ]
...
def debug_lists_and_dictionaries_thread():
	...
	if all_settings_dict != {}:
		...
		values_read_from_configfile.append('mpeg_nonfree_wrapper_formats = ' + str(mpeg_nonfree_wrapper_formats))
	...
	try:
		...
		for counter in range(0, number_of_ffmpeg_supported_audiostreams):
			...
			if len(ffmpeg_commandline) == 0:
				...
				# Create start of the commandline for FFmpeg >= 2.8.6 in case the input file is mpegts
				if file_type == 'mpegts':
...
try:
	...
	if configfile_path != '':
		...
		if 'mpeg_nonfree_wrapper_formats' in all_settings_dict:
			mpeg_nonfree_wrapper_formats = all_settings_dict['mpeg_nonfree_wrapper_formats']
		...
		if 'server_name' in all_settings_dict:
			server_name = all_settings_dict['server_name']

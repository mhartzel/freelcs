...
RUN ldconfig
RUN chmod 755 /usr/bin/LoudnessCorrection.py /usr/bin/loudness-freelcs /usr/bin/freelcs_startup_file.sh ; chmod 644 /etc/Loudness_Correction_Settings.json /etc/samba/smb.conf /usr/lib/libinput_sndfile-freelcs.so
...
COPY ./Loudness_Correction_Settings.json /etc/
RUN chmod 755 /usr/bin/progress_report ; chmod 644 /etc/Loudness_Correction_Settings.json
CMD ["/usr/bin/progress_report", "-configfile", "/etc/Loudness_Correction_Settings.json"]
...
COPY ./Loudness_Correction_Settings.json /etc/
RUN chmod 755 /usr/bin/heartbeat_checker ; chmod 644 /etc/Loudness_Correction_Settings.json
CMD ["/usr/bin/heartbeat_checker", "-configfile", "/etc/Loudness_Correction_Settings.json"]
